...
- Concern-specific middleware comes from `go-http-middleware`,
  `go-authentication`, `go-authorization`, `go-rate-limit`,
  `go-idempotency`, and `go-telemetry`.
...
- Route metadata MAY support `go-openrpc`, documentation, telemetry, and
...
- `go-service` owns HTTP server and process lifecycle and accepts the compiled
  router as an `http.Handler`.
- `go-http-middleware` owns reusable generic server middleware.
...
- `go-jsonrpc` supplies an HTTP handler that can be mounted at an explicit
  route; JSON-RPC methods are not HTTP routes.
- `go-openrpc` MAY consume route metadata through an optional adapter.
- `go-webhook` handlers mount explicitly and select their own authentication.
...
- integration tests with `go-service`, `go-http-middleware`, `go-jsonrpc`, and
